...
**⚠️ IMPORTANTE**: Este proyecto usa **Maven Wrapper** (incluido), NO Maven global. Los archivos `.mvn/wrapper/maven-wrapper.jar` y `mvnw.cmd` están en el repositorio.

- `.\mvnw.cmd clean install`: Compile, run Surefire + Failsafe, and emit `target/ROOT.war`.
- `.\mvnw.cmd spring-boot:run -Pboot`: Start the server on port 8080 with hot reload-friendly Boot profile.
- `.\mvnw.cmd clean package spring-boot:repackage -Pboot && java -jar target/ROOT.war`: Build and exercise the bootable WAR.
...
## Testing Guidelines
- `.\mvnw.cmd test`: Runs JUnit Jupiter unit suites such as `CustomBeanTest` and `ParallelUpdatesVersionConflictTest`.
- `.\mvnw.cmd verify`: Adds integration coverage through Failsafe with the default H2 datasource; if you pivot to PostgreSQL, run `.\mvnw.cmd install -DskipTests` until fixtures are updated.
...
- Run `.\mvnw.cmd verify` or the relevant Docker workflow before review; note any skipped checks and how to reproduce the result.
